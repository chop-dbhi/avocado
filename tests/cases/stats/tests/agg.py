...
class AggregatorTestCase(TestCase):
    ...
    def test_count(self):
        self.assertEqual(self.is_manager.count(distinct=True), 2)
        ...
        self.assertEqual(
            self.is_manager.count(queryset=queryset), 1)
    ...
    def test_max(self):
        ...
        self.assertEqual(self.salary.max(queryset=queryset), 100000)
    ...
    def test_min(self):
        ...
        self.assertEqual(self.salary.min(queryset=queryset), 200000)
    ...
    def test_avg(self):
        ...
        self.assertEqual(self.salary.avg(queryset=queryset), 150000)
    ...
    def test_sum(self):
        ...
        queryset = self.salary.model.objects.filter(salary__gte=100000)
        self.assertEqual(self.salary.sum(queryset=queryset), 300000)
    ...
    if sys.version_info >= (2, 7):
        ...
        def test_stddev(self):
            ...
            self.assertEqual(
                self.salary.stddev(queryset=queryset), 2500)
    else:
        def test_stddev(self):
            self.assertRaises(DatabaseError, self.is_manager.stddev())
            self.assertRaises(DatabaseError, self.salary.stddev)
    ...
    if sys.version_info >= (2, 7):
        ...
        def test_variance(self):
            ...
            self.assertRaises(DatabaseError, self.salary.variance)
