...
class Command(DataFieldCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option(
            '--flush', action='store_true', help='Flushes existing cache for '
            'each cached property.'),

        make_option(
            '--method', action='append', dest='methods',
            default=CACHED_METHODS, help='Select which methods to pre-cache. '
            'Choices: {0}'.format(', '.join(CACHED_METHODS))),
    )
    ...
    def handle_fields(self, fields, **options):
        ...
        for method in methods:
            if method not in CACHED_METHODS:
                raise CommandError(
                    'Invalid method {0}. Choices are {1}'
        ...
        for f in fields:
            ...
            self._progress()
            log.debug(
                '{0} cache set took {1} seconds'.format(f, time.time() - t0))

        print(u'\n{0} fields have been updated ({1} s)'
              .format(count, round(time.time() - t0, 2)))
