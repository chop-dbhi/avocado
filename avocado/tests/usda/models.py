...
class FoodDescription(models.Model):
    ndb_no = models.CharField("Nutrient Database Number", max_length=5,
            primary_key=True)
    ...
    refuse_desc = models.CharField("Inedible Description", max_length=135,
            blank=True)
    refuse = models.IntegerField("Percent Refuse", blank=True)
    sci_name = models.CharField("Scientific Name", max_length=65, blank=True)
    n_factor = models.DecimalField("Nitrogen Converting Factor", max_digits=6,
            decimal_places=2, blank=True)
    pro_factor = models.DecimalField("Calories from protein factor",
            max_digits=6, decimal_places=2, blank=True)
    fat_factor = models.DecimalField("Calories from fat factor", max_digits=6,
            decimal_places=2, blank=True)
    cho_factor = models.DecimalField("Calories from carbs factor", max_digits=6,
            decimal_places=2, blank=True)
...
class NutrientData(models.Model):
    ...
    nutr_val = models.DecimalField("Amount in 100 grams", max_digits=13,
            decimal_places=3)
    ...
    add_nutr_mark = models.NullBooleanField("Vitamin added for enrichment",
            blank=True)
    ...
    low_eb = models.DecimalField("Lower 95% \error bound", max_digits=13,
            decimal_places=3, blank=True)
    up_eb = models.DecimalField("Upper 95% \error bound", max_digits=13,
            decimal_places=3, blank=True)
    stat_comment = models.CommaSeparatedIntegerField(
            max_length=10, blank=True)

class NutrientField(models.Model):
    nutr_no = models.CharField("Nutrient Identifier", max_length=3,
            primary_key=True)
...
#    amount = models.DecimalField("Unit Modifier", max_digits=8,
#            decimal_places=3)
...
#    nutr_no = models.ForeignKey('NutrientField', null=True, blank=True,
#            default=None)
