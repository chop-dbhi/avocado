...
def dedupe(_values):
    values = _values[:]
    # Hash of duplicates by value to an array of indexes
    duplicates = defaultdict(list)

    previous = None

    for i, value in enumerate(values):
        # Detect duplicate order value
    ...
    for value in duplicates:
        indexes = duplicates[value]

        # If no high value exists, increment all values by 1,
        # otherwise divide the length of values into the difference
        if indexes[-1] >= len(values) - 1:
            incr = 1
        else:
            incr = (values[indexes[-1] + 1] - value) / float(len(indexes) + 1)

        # Determine midpoint relative to the next (non-duplicate)
        # order. The last duplicate order will have a 1 added
        for i, index in enumerate(indexes):
            values[index] += incr * (i + 1)
...
class Migration(DataMigration):
    def forwards(self, orm):
        multi_field_concepts = orm.DataConcept.objects\
            .annotate(field_count=Count('fields'))\
            .filter(field_count__gt=1)

        for c in multi_field_concepts:
            orders = []
            cfields = tuple(c.concept_fields.order_by('order', 'created'))

            for cf in cfields:
                orders.append(cf.order or 0)

            for i, order in enumerate(dedupe(orders)):
                cf = cfields[i]
    ...
    models = {
        ...
        'avocado.datacontext': {
            ...
            'accessed': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 3, 6, 0, 0)'}),
