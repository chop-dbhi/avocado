...
class ColumnConceptCache(ConceptCache):
    def get(self, concept_id, queryset=None, ret_val=None):
        ...
        return super(ColumnConceptCache, self).get(concept_id=concept_id,
            queryset=queryset, ret_val=ret_val)
    
    def get_fields(self, concept_id, queryset=None, ret_val=None):
        if queryset is None:
            queryset = ColumnConcept.objects.all()
        return super(ColumnConceptCache, self).get_fields(concept_id=concept_id,
            queryset=queryset, ret_val=ret_val)


cache = ColumnConceptCache(ColumnConcept.__name__.lower())