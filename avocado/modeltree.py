...
class ModelTree(object):
    ...
    `_model_name_cache' contains names of the all the models that have already
    been iterated over in the tree. this prevents circular dependencies as well
    as unwanted or unecessary processing.
    """
    ...
    def _add_node(self, parent, model, rel_type, rel_is_reversed, related_name,
        ...
        # TODO add some kind of coercian technique to choose a path a node should
        # traverse. this is necessary for certain kinds of joins either for
        # performance reasons or general preference
    ...
    def _find_relations(self, node, depth=0):
        ...
        'through' models being evaluated as a ForeignKey relationship.
        """
        depth += 1
        ...
        for f in m2m_fields:
            model = f.rel.to
            self._add_node(node, model, 'manytomany', False, f.name, f.name,
                depth)

        # iterate over related m2m fields
        for f in related_m2m:
            model = f.model
            self._add_node(node, model, 'manytomany', True, f.var_name,
                f.get_accessor_name(), depth)

        # iterate over one2one fields
        for f in o2o_on_model:
            model = f.rel.to
            self._add_node(node, model, 'onetoone', False, f.name, f.name,
                depth)

        # iterate over related one2one fields
        for f in related_o2o:
            model = f.model
            self._add_node(node, model, 'onetoone', True, f.get_accessor_name(),
                f.get_accessor_name(), depth)

        # iterate over fk fields
        for f in fk_on_model:
            model = f.rel.to
            self._add_node(node, model, 'foreignkey', False, f.name, f.name,
                depth)

        # iterate over related foreign keys
        for f in related_fk:
            model = f.model
            self._add_node(node, model, 'foreignkey', True, f.var_name,
                f.get_accessor_name(), depth)
